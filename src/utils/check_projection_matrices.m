...
function check_projection_matrices(P0, P1, P2, P3, r, basis_name)
    % Check basic properties of discretization matrices
    fprintf('\n=== Discretization Validation (%s) ===\n', basis_name);
    fprintf('P0 size: %dx%d\n', size(P0));
    fprintf('P1 size: %dx%d\n', size(P1));
    fprintf('P2 size: %dx%d\n', size(P2));
    fprintf('P3 size: %dx%d\n', size(P3));

    % Test derivative consistency on random coefficient vectors
    dof = size(P0, 2);
    test_coeffs = randn(dof, 3);
    ...
    for i = 1:3
        ...
        f_prime_num  = gradient(f, r);
        f_dprime_num = gradient(f_prime_num, r);
        f_tprime_num = gradient(f_dprime_num, r);

        fprintf('\nTest function %d:\n', i);
        rel_error1 = norm(f_prime  - f_prime_num)  / max(norm(f_prime_num),  eps);
        rel_error2 = norm(f_dprime - f_dprime_num) / max(norm(f_dprime_num), eps);
        rel_error3 = norm(f_tprime - f_tprime_num) / max(norm(f_tprime_num), eps);

        fprintf('  ||P1*f - numerical f''||/||f''||   : %.2e\n', rel_error1);
        fprintf('  ||P2*f - numerical f''''||/||f''''|| : %.2e\n', rel_error2);
        fprintf('  ||P3*f - numerical f''''''||/||f''''''||: %.2e\n', rel_error3);

        % Plot f
        subplot(3, 4, (i-1)*4 + 1);
        plot(r, f, 'b-', 'LineWidth', 2);
        title(sprintf('Test %d: f(r)', i));
        xlabel('r'); ylabel('f(r)');
        grid on;
        ...
        plot(r, f_prime - f_prime_num, 'g:', 'LineWidth', 1, 'DisplayName', 'Difference');
        hold off;
        title(sprintf('First derivative (err: %.1e)', rel_error1));
        xlabel('r'); ylabel('f''(r)');
        legend('Location', 'best');
        grid on;
        ...
        plot(r, f_dprime - f_dprime_num, 'g:', 'LineWidth', 1, 'DisplayName', 'Difference');
        hold off;
        title(sprintf('Second derivative (err: %.1e)', rel_error2));
        xlabel('r'); ylabel('f''''(r)');
        legend('Location', 'best');
        grid on;

        % Plot third derivative comparison
        subplot(3, 4, (i-1)*4 + 4);
        plot(r, f_tprime, 'r-', 'LineWidth', 2, 'DisplayName', 'P3*f');
        hold on;
        plot(r, f_tprime_num, 'k--', 'LineWidth', 1.5, 'DisplayName', 'Numerical f''''''');
        plot(r, f_tprime - f_tprime_num, 'g:', 'LineWidth', 1, 'DisplayName', 'Difference');
        hold off;
        title(sprintf('Third derivative (err: %.1e)', rel_error3));
        xlabel('r'); ylabel('f^{(3)}(r)');
        legend('Location', 'best');
        grid on;
    end
    ...
    % Check null spaces / conditioning
    fprintf('\nNull space checks:\n');
    fprintf('  rank(P0): %d (max possible %d)\n', rank(full(P0)), min(size(P0)));
    fprintf('  rank(P1): %d (max possible %d)\n', rank(full(P1)), min(size(P1)));
    fprintf('  rank(P2): %d (max possible %d)\n', rank(full(P2)), min(size(P2)));
    fprintf('  rank(P3): %d (max possible %d)\n', rank(full(P3)), min(size(P3)));
    ...
    % Additional diagnostics against finite-difference differentiation
    G = gradient_matrix(r);
    fprintf('\nAdditional diagnostics:\n');
    fprintf('  ||P1 - G*P0||_F / ||P1||_F: %.2e\n', norm(P1 - G * P0, 'fro') / max(norm(P1, 'fro'), eps));
    fprintf('  ||P2 - G*P1||_F / ||P2||_F: %.2e\n', norm(P2 - G * P1, 'fro') / max(norm(P2, 'fro'), eps));
    fprintf('  ||P3 - G*P2||_F / ||P3||_F: %.2e\n', norm(P3 - G * P2, 'fro') / max(norm(P3, 'fro'), eps));
    ...
    subplot(1, 4, 4);
    spy(P3);
    title(sprintf('P3 Sparsity (nnz=%d, %.1f%%)', nnz(P3), 100*nnz(P3)/numel(P3)));
    xlabel('Coefficients'); ylabel('Radial Points');
...
function G = gradient_matrix(r)
    ...
    % Use local spacing if r is nonuniform
    for i = 2:n-1
        hL = r(i)   - r(i-1);
        hR = r(i+1) - r(i);
        G(i, i-1) = -hR / (hL * (hL + hR));
    ...
    % One-sided first-order boundaries
    G(1, 1) = -1/(r(2) - r(1));
    G(1, 2) =  1/(r(2) - r(1));
    G(n, n-1) = -1/(r(n) - r(n-1));
    G(n, n)   =  1/(r(n) - r(n-1));
