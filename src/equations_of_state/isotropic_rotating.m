function [dbetapardB0, dbetapardr, dbetapardB, dbetapardR, d2betapardB2, ...
    ...
    d3betapardrdR2, betapar, betaperp] = isotropic_rotating(kinetic_profiles,r,RR,BB,varargin)
    ...
    if nargin > 4
        d2betapardrdR = 2.*exp(mach2.*(-1 + RR.^2)).*RR.*( ...
            beta.*mach2p + mach2.*(betap ...
            +beta.*mach2p.*(-1 + RR.^2)));
        d2betapardR2 = 2.*beta.*exp(mach2.*(-1 + RR.^2)).*mach2.*( ...
            1+2.*mach2.*RR.^2);
        ...
        dbetapardB0 = variational_pressure_output( ...
            ...
            d2betapardrdR,d2betapardR2,betapar);
        return
    end
    if nargout > 7 % Jacobian computation
        ...
        d3betapardrdR2 = 2.*exp(mach2.*(-1 + RR.^2)).*(beta.*mach2p + 2.*mach2.^2.*RR.^2.*(betap + beta.*mach2p.*(-1 + RR.^2)) + mach2.*(betap + beta.*mach2p.*(-1 + 5.*RR.^2)));
