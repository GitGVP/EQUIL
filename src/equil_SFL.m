function LY = equil_SFL(LX, LY, r_plt, RR, ZZ, delta, deltap, deltapp, P_hat, Pp, Ppp, S, Sp, Spp, ms)
  ...
  Z1 = 1i *l1;
  Y1 = l1p;
  LY.Y0 = - (r_plt < rs) ./LY.N0 .* ( LX.eps_val .* (signn * 1i .* LY.JoverBm1 + LY.Mm1) .* Z1 + LY.Nm1 .* Y1);
  LY.Y0_LO = real((1i * signn * LX.qfun(r_plt) .* LX.kinetic_profiles.betap(r_plt) .* Z1 ./ r_plt - (1i/2) * (deltapp + deltap ./ r_plt + 1) .* Z1 - Y1 .* deltap)) .* (r_plt < rs);
