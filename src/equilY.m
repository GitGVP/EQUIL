function LY = equilY(L, LX)
  ...
  LY.deltap_fine = LX.qfun(r_fine).^2 ./ r_fine .^ 3 .* cumtrapz(r_fine, r_fine.^3 ./ LX.qfun(r_fine).^2 - 2 * r_fine.^2 .* dbetapardr + r_fine.^2 .* d2betapardrdB );
  ...
  %NLO shift
  delta1_RHS = -((3.*(-d2betapardrdB + 2.*dbetapardr).*r_fine.*(LY.S2_fine + r_fine.*LY.S2p_fine))./2. + (LX.qpfun(r_fine).*r_fine ...
    .*(-2.*LY.S2_fine.*LY.S3_fine + 3.*r_fine.^2.*LY.S2p_fine.*(-LY.deltap_fine + LY.S3p_fine)))./LX.qfun(r_fine).^3 + (2.*(7.*LY.S2_fine + 5.*r_fine.*LY.S2p_fine).*LY.S3_fine - r_fine...
    .*(LY.S2_fine.*(3.*LY.deltap_fine - 5.*LY.S3p_fine) + r_fine.*LY.S2p_fine.*(-3.*LY.deltap_fine + r_fine + LY.S3p_fine)))./LX.qfun(r_fine).^2);
