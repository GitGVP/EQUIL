function R = residuals_rotation(r,x,epsilon,omega,kinetic_profiles,q,qp,P0,P1,P2,M_profiles,dof_count,Nb,Sbc,equation_of_state)
	N = numel(r);
	Ns = numel(Sbc);
	nRes = 3+Nb+Ns;
	t2_c    = x(1:dof_count);
	...
	T21 = sum(-((-1 + ms).*S.*sin((-1 + ms).*omega)),3).*(-(deltap.*epsilon) + cos(omega) + epsilon.^2.*Pp.*cos(omega) + epsilon.*sum(cos((-1 + ms).*omega).*Sp,3)) - sum((-1 + ms).*cos((-1 + ms).*omega).*S,3).*(sin(omega) + epsilon.^2.*Pp.*sin(omega) - epsilon.*sum(sin((-1 + ms).*omega).*Sp,3)) - (epsilon.^2.*P + r).*(-(deltap.*sin(omega)) + sin(omega).*sum(cos((-1 + ms).*omega).*Sp,3) + cos(omega).*sum(sin((-1 + ms).*omega).*Sp,3));
	T26 = d2betapardB2.*sum(-(mb.*Bs.*sin(mb.*omega)),3) - d2betapardRdB.*(epsilon.^2.*P.*sin(omega) + r.*sin(omega) - epsilon.*sum(-((-1 + ms).*S.*sin((-1 + ms).*omega)),3));
	T27 = d2betapardrdB + d2betapardB2.*epsilon.*sum(Bsp.*cos(mb.*omega),3) + d2betapardRdB.*epsilon.*(-(deltap.*epsilon) + cos(omega) + epsilon.^2.*Pp.*cos(omega) + epsilon.*sum(cos((-1 + ms).*omega).*Sp,3));
	T29 = -(epsilon.^2.*P.*cos(omega)) - r.*cos(omega) + epsilon.*sum(-((-1 + ms).^2.*cos((-1 + ms).*omega).*S),3);
	T31 = -(epsilon.^2.*P.*sin(omega)) - r.*sin(omega) - epsilon.*sum(-((-1 + ms).^2.*S.*sin((-1 + ms).*omega)),3);

	resF = (1 - delta.*epsilon.^2 + epsilon.^3.*P.*cos(omega) + epsilon.*r.*cos(omega) + epsilon.^2.*sum(cos((-1 + ms).*omega).*S,3)).*((epsilon.^2.*P.*cos(omega) + r.*cos(omega) - epsilon.*sum((-1 + ms).*cos((-1 + ms).*omega).*S,3)).*(-(deltap.*epsilon) + cos(omega) + epsilon.^2.*Pp.*cos(omega) + epsilon.*sum(cos((-1 + ms).*omega).*Sp,3)) + (epsilon.^2.*P.*sin(omega) + r.*sin(omega) - epsilon.*sum(-((-1 + ms).*S.*sin((-1 + ms).*omega)),3)).*(sin(omega) + epsilon.^2.*Pp.*sin(omega) - epsilon.*sum(sin((-1 + ms).*omega).*Sp,3))).*(-(dbetapardR.*epsilon.*T3) - (r.^2.*T5.^2.*T9.*(-(epsilon.^2.*T1.*T21.*T9) + RR.*(epsilon.*T1.*T16.*T18.*T21 - epsilon.*T17.*T18.*T2.*T21 - epsilon.*T18.*T21.*T3.*T31 + epsilon.*T18.*T21.*T29.*T4 + T1.*T17.*T9 + T16.*T2.*T9 + T29.*T3.*T9 + T31.*T4.*T9)))./(q.^2.*RR.^3) + (BB.*(dbetapardr - BB.*T27 + dbetapardR.*epsilon.*T3 + dbetapardB.*epsilon.*sum(Bsp.*cos(mb.*omega),3)))./(BB - dbetapardB.*epsilon.^2) + (BB.*T5.*(BB.*(BB - dbetapardB.*epsilon.^2).*t2p + T5.*(BB.*T27 - dbetapardB.*epsilon.*sum(Bsp.*cos(mb.*omega),3))))./((BB - dbetapardB.*epsilon.^2).^3.*RR.^2) + (BB.*epsilon.^2.*T21.*(-(dbetapardR.*(BB - dbetapardB.*epsilon.^2).^2.*RR.^2.*T1) + (BB.^2.*RR.^2 - 2.*BB.*dbetapardB.*epsilon.^2.*RR.^2 + dbetapardB.^2.*epsilon.^4.*RR.^2 - T13).*(BB.*T26 - dbetapardB.*sum(-(mb.*Bs.*sin(mb.*omega)),3))))./((BB - dbetapardB.*epsilon.^2).^3.*RR.^2.*T12) + (r.*T5.*T9.*(-(qp.*r.*RR.*T12.*T5.*T9) + q.*(deltapp.*epsilon.*r.*RR.*T12.*T18.*T2.*T5 - r.*RR.*T12.*T16.*T18.*T3.*T5 + r.*RR.*T12.*T17.*T18.*T4.*T5 + epsilon.^2.*r.*RR.*T12.*t2p.*T9 + RR.*T12.*T5.*T9 + 2.*r.*RR.*T1.*T17.*T5.*T9 + 2.*r.*RR.*T16.*T2.*T5.*T9 - epsilon.*r.*T12.*T3.*T5.*T9 - epsilon.^2.*Ppp.*r.*RR.*T12.*T18.*T2.*T5.*cos(omega) + epsilon.^2.*Ppp.*r.*RR.*T1.*T12.*T18.*T5.*sin(omega) - epsilon.*r.*RR.*T12.*T18.*T2.*T5.*sum(cos((-1 + ms).*omega).*Spp,3) - epsilon.*r.*RR.*T1.*T12.*T18.*T5.*sum(sin((-1 + ms).*omega).*Spp,3))))./(q.^3.*RR.^3));
    resB = (BB.^2.*(1 - T13./((BB - dbetapardB.*epsilon.^2).^2.*RR.^2)))./epsilon - (epsilon.*r.^2.*T12.*T13.*T18)./(q.^2.*RR.^2);
	...
	resTotal(3,:) = mean((r - T8./RR)./epsilon.^2, 2);
