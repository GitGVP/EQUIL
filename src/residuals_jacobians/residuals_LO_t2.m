function R = residuals_LO_t2(r,x,epsilon,omega,kinetic_profiles,q,qp,P0,P1,P2,M_profiles,dof_count,Nb,Sbc,equation_of_state)
	N = numel(r);
	Ns = numel(Sbc);
	nRes = 3+Nb+Ns;
	t2_c    = x(1:dof_count);
	...
	resF = 2.*q.*r - qp.*r.^2 + q.^3.*(dbetapardB + dbetapardr - d2betapardrdB.*r + t2p) + delta.*cos(omega) + sum(cos(ms.*omega).*(-r.^(-3 + ms) + Spp),3);
