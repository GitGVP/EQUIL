...
load /home/vanparys/Documents/PhD/Codes/MatlabEQ/TCV_limiter.mat
...
contourf(ax4, R0*LY4.RR, R0*LY4.ZZ, LY4.betapar,20,'LineColor','none');
for i=floor(linspace(1,numel(LY4.r_plt),7)); plot(ax4,R0*LY4.RR(i,:), R0*LY4.ZZ(i,:),'--','Color', [1,1,1,0.7],'LineWidth',0.5); end
...
contourf(ax5, R0*LY2.RR, R0*LY2.ZZ, (LY2.betapar+LY2.betaperp)/2,20,'LineColor','none');
for i=floor(linspace(1,numel(LY2.r_plt),7)); plot(ax5,R0*LY2.RR(i,:), R0*LY2.ZZ(i,:),'--','Color', [1,1,1,0.7],'LineWidth',0.5); end
...
% First 3 plots horizontal together




% Redo this but split the plots
tl = tiledlayout(1,3,'TileSpacing','compact','Padding','compact');

%% Left column: three stacked small plots, each 2 rows high
ax1 = nexttile(1); % rows 1-2, col 1
hold(ax1,'on');
h1 = plot(ax1, L3.r_q, LY3.gavg-1, '-','LineWidth',3); % no pressure first
h2 = plot(ax1, L.r_q,  LY.gavg-1, '--','LineWidth',3);
h3 = plot(ax1, L2.r_q, LY2.gavg-1, '-.','LineWidth',3);
...
%leg = legend(ax1,[h1 h2], {'$p=0$', '$p_\perp=0, \mathcal M^2=1$'}, ...
%   'Interpreter','latex','Box','off','FontSize',12, 'Location', 'SouthWest');
%leg2 = legend(ax2,[h3 h4], {'isotropic $\mathcal M^2=1$',...
%    '$p_\perp=0, \mathcal M^2=0$'}, ...
%    'Interpreter','latex','Box','off','FontSize',12,'Location','NorthWest');
st = sgtitle(' ');  % blank title (keeps appearance unchanged)
drawnow;

% --- read tiledlayout position in normalized figure units ---
tlPos = tl.Position;  % [x y w h]

% choose the title / legend row height (tweak if needed)
titleHeight = 0.06;   % normalized units (about 6% of figure height). adjust if necessary

% compute invisible-axes position inside the figure such that it overlaps the top of tiledlayout
% we place it so it sits within the tiledlayout top row (no padding change required)
axLegPos = [tlPos(1), tlPos(2) + tlPos(4) - titleHeight, tlPos(3), titleHeight];

% create invisible axes to host legend
axLeg = axes('Position', axLegPos, 'Visible', 'off', 'HitTest', 'off');

% create legend attached to invisible axes, horizontal and centered
lg = legend(axLeg, [h1 h2 h3 h4], ...
    {'$p=0$', '$p_\perp=0,\ \mathcal M^2=1$', 'isotropic $\mathcal M^2=1$', '$p_\perp=0,\ \mathcal M^2=0$'}, ...
...
% center legend inside axLeg by adjusting its Position relative to axLeg
drawnow;                  % ensure lg.Position is up-to-date
legPos = lg.Position;     % relative to figure
axPos  = axLeg.Position;  % relative to figure

% compute centered x inside axLeg
newX = axPos(1) + (axPos(3) - legPos(3))/2;
% compute y so legend is vertically centered in the small title area
newY = axPos(2) + (axPos(4) - legPos(4))/2;

% set new legend position (keep width/height)
...
contourf(ax1, R0*LY.RR, R0*LY.ZZ, LY.betapar,20,'LineColor','none');
for i=floor(linspace(1,numel(LY.r_plt),7)); plot(ax1,R0*LY.RR(i,:), R0*LY.ZZ(i,:),'--','Color', [1,1,1,0.7],'LineWidth',0.5); end
...
%% Test with LY to check if the contours will be exactly the same
...
axis(ax4,'equal'); hold(ax4,'on');
plot(ax4,rl,zl,'color',0.3*[1 1 1],'linewidth',3);
contourf(ax4, R0*LY.RR, R0*LY.ZZ, LY.betapar,20,'LineColor','none');
for i=floor(linspace(1,numel(LY.r_plt),7)); plot(ax4,R0*LY.RR(i,:), R0*LY.ZZ(i,:),'--','Color', [1,1,1,0.7],'LineWidth',0.5); end
plot(ax4,R0*LY.RR(end,:), R0*LY.ZZ(end,:), 'k', 'LineWidth',4);
title(ax4,'$\beta_\parallel$   $(\beta_\perp=0,\mathcal M^2=1)$','Interpreter','latex','FontSize',14);
colorbar(ax4,'eastoutside');
xlabel(ax4,'$R$ [m]','Interpreter','latex','FontSize',14);
ylabel(ax4,'$Z$ [m]','Interpreter','latex','FontSize',14);
ax5 = nexttile(2); 
...
%% Test circular cross section, elongation driven by pressure
%s0 = 2.5;
%beta = 2.7; %2.7 is max
...
beta_list = [1.2, 1.4, 1.6, 1.8, 2, 2.2];

% First solve (no hot restart)
[L, LX] = equilSol('beta', beta_list(1), 's0', s0, 'debug', 4, ...
    'residuals_fun', @residuals_noRepl, 'jacobian_fun', @jacobian_noRepl, ...
    'equation_of_state', @isotropic, 'Nb', 1, 'Ns', 2);
...
% Subsequent solves (hot restart)
for k = 2:numel(beta_list)
    beta = beta_list(k);
...
figure; hold on;
plot(L.r_q, (squeeze(LY.S(2:end,:,1)) - LY.S2_ana)/eps_val, '-','LineWidth',3)
plot(LY.r_fine, LY.S2_1_fine, '-','LineWidth',3)
...
p = plot(LY.r_fine, LY.S2_1_fine, '--', 'LineWidth', 2);
plot(L.r_q, (squeeze(LY.S(2:end,:,1)) - LY.S2_ana)/eps_val, ...
    '.', 'Color', p.Color);
